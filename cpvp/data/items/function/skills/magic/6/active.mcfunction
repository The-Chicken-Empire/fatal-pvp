...
summon marker ~ ~ ~ {Tags:["skillmarker","magic6"]}
...
execute as @e[tag=skillmarker,tag=magic6,sort=nearest,limit=1] at @s run particle soul_fire_flame ~ ~1.5 ~ 0 0 0 0.3 10 force
execute as @e[tag=magic6,type=marker,tag=skillmarker,limit=1] at @s positioned ~0 ~1 ~0 as @e[tag=!skillmarker,dx=0] unless score @s teamscore = @e[tag=skillmarker,tag=magic6,limit=1] ownerteam positioned ~-0.99 ~0.01 ~-0.99 if entity @s[dx=0] run tag @s add magic6victim
