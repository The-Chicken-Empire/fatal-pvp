...
execute at @s positioned ^ ^ ^0.2 positioned ~-0.1 ~-0.1 ~-0.1 as @a[tag=playing,dx=0] positioned ~-0.8 ~-0.8 ~-0.8 if entity @s[dx=0] unless score @s teamscore = @e[tag=magic13tmp5,limit=1] ownerteam run return run function items:skills/magic/13/hit
