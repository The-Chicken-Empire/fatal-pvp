...
execute as @e[tag=meleeatkertmp2] if score @s playerdata = @e[tag=victimtmp,limit=1] meleeattacker run tag @s add meleeatkertmp3
...
#####test
data modify storage test test set from storage cpvp:attackdatae
execute if entity @s[type=player] store result storage cpvp:tmp player byte 1 run scoreboard players get @s playerdata
execute if entity @s[type=!player] store result storage cpvp:tmp entity byte 1 run scoreboard players get @s entitydata
execute if entity @s[tag=meleedamage,type=player] run function damage:atktrigger/melee/p with storage cpvp:tmp
